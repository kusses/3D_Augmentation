...
class RandomRotateTargetAngle(object):
    ...
    def __call__(self, data_dict):
        ...
        if "normal" in data_dict.keys():
            data_dict["normal"] = np.dot(data_dict["normal"], np.transpose(rot_t))
        # (jy) preserve 'segment' key
        #if "segment" not in data_dict:
        #    print("there isn't 'segment' key.")
        return data_dict
...
class Compose(object):
    ...
    def __call__(self, data_dict):
        for t in self.transforms:
            # print(f"Data dict before applying {t.__class__.__name__}: {data_dict.keys()}")
            data_dict = t(data_dict)
            # print(f"Data dict after applying {t.__class__.__name__}: {data_dict.keys()}")
        return data_dict
