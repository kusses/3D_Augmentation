    ...
    def test(self):
        ...
        if comm.is_main_process():
            ...
            if self.cfg.data.test.type == "volmeda":
                ...
                f1_stats = compute_f1_from_confusion(intersection, union, target)
                logger.info(
                    "Val F1 (hard): macroF1/microF1 {:.4f}/{:.4f} "
                    "(macroP {:.4f}, macroR {:.4f}; microP {:.4f}, microR {:.4f})".format(
                        f1_stats["macro_f1"], f1_stats["micro_f1"],
                        f1_stats["macro_precision"], f1_stats["macro_recall"],
                        f1_stats["micro_precision"], f1_stats["micro_recall"]
                    )
                )
                if hasattr(self, "writer") and self.writer is not None and hasattr(self, "epoch"):
                    self.writer.add_scalar("Val/F1_macro_hard", f1_stats["macro_f1"], self.epoch)
                    self.writer.add_scalar("Val/F1_micro_hard", f1_stats["micro_f1"], self.epoch)

                for i in range(self.cfg.data.num_classes):
                    logger.info(
                        "Class_{idx} - {name} F1 (hard): {f1:.4f}  P {p:.4f}  R {r:.4f}".format(
                            idx=i, name=self.cfg.data.names[i],
                            f1=f1_stats["f1_class"][i],
                            p=f1_stats["precision_class"][i],
                            r=f1_stats["recall_class"][i],
                        )
                    )
                    if hasattr(self, "writer") and self.writer is not None and hasattr(self, "epoch"):
                        cname = self.cfg.data.names[i]
                        self.writer.add_scalar(f"Val/F1_hard/{cname}", f1_stats["f1_class"][i], self.epoch)
                        self.writer.add_scalar(f"Val/P_hard/{cname}", f1_stats["precision_class"][i], self.epoch)
                        self.writer.add_scalar(f"Val/R_hard/{cname}", f1_stats["recall_class"][i], self.epoch)
