...
output_folder_path = '/data/volme/data/VOLMEDA/modified'
...
for ply_file in ply_files:
    ...
    # Create a dictionary to store new vertex data by pred_class
    vertex_data_by_class = {}
    for vertex in new_vertex_data:
        pred_class = vertex[-1]
        if pred_class not in vertex_data_by_class:
            vertex_data_by_class[pred_class] = []
        vertex_data_by_class[pred_class].append(vertex)

    # Ensure the output subdirectory exists
    class_output_folder = os.path.join(output_folder_path, base_name)
    os.makedirs(class_output_folder, exist_ok=True)

    # Create and save new PLY files for each pred_class
    for pred_class, class_vertex_data in vertex_data_by_class.items():
        class_vertex_data_np = np.array(class_vertex_data, dtype=new_vertex_data.dtype)
        new_vertex_element = plyfile.PlyElement.describe(class_vertex_data_np, 'vertex')
        elements_list = list(ply_data.elements)
        elements_list[0] = new_vertex_element
        new_ply_data = plyfile.PlyData(elements_list, text=True)
        new_ply_file_path = os.path.join(class_output_folder, f"{base_name}_pred_{pred_class}.ply")
