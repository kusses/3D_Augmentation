# Fashion 3D Data Augmentation - Coord and Color augmentation

This project preprocesses 3D point cloud data(.ply) and augments them for model learning. 
...
* note: this code is not perfectly reviewed and edited for this project.
...
There are 3 types of main files: file, folder, and folder plan.
...
### 4. Option
The augmented files need to be moved with each model inference.
For example, Files should be placed separately in scene_id.ply folders for the Ponitcept model inference.
Set the folder inside the code.
The original files should be copied to the destination root
<example>
...
python Move_augfile.py
 ```
Data split text(train.txt, val.txt, test.txt) will be generated by these codes
For original data
 ```
...
The data is split and placed in each dataset in the train, eval, and test folders according to plan.
 ```
